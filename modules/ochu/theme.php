...
function nv_theme_samples_main ( $sql_data, $now_page )
{
	...
	{
		$xtpl->assign( 'URL_VIEW', NV_BASE_SITEURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" . NV_NAME_VARIABLE . "=" . $module_name."&amp;" . NV_OP_VARIABLE . "=view&amp;id=".$row['id']."");
...
function nv_theme_onbai_test( $key, $suggest, $data, $info, $do )
{
	...
	{	
		...
		{
			// viet cac o chu
			if ( ( $key[$stt] -1 ) == $td_stt ) $xtpl->assign( 'KEYCLASS', 'key' );
			else $xtpl->assign( 'KEYCLASS', 'cell' );
