...
if ( $nv_Request->get_int( 'edit', 'post', 0 ) == 1 )
{
	...
	{	
		$query = $db->query("UPDATE `" . NV_PREFIXLANG . "_" . $module_data . "` SET `".$key."` = " . $db->dbescape( $data ) . " WHERE `id` =" . $id . "");
	}
	if ( $query ) 
	{
...
{	
	...
	// lay theo post
	$rowcontent['title'] = $nv_Request->get_string( 'title', 'post', '' );
	$rowcontent['content'] = $nv_Request->get_string( 'content', 'post', '' );
	$rowcontent['key'] = $nv_Request->get_string( 'key', 'post', '' );
	$rowcontent['quession'] = $nv_Request->get_string( 'quession', 'post', '' );
	...
	{
		$query = "INSERT INTO " . NV_PREFIXLANG . "_" . $module_data . " (`title`,`content`,`key`,`question`) VALUES 
		( 
			" . $rowcontent['title'] . ", 
			`" .  $rowcontent['content']  . "`, 
			`" .  $rowcontent['key']  . "` ,
